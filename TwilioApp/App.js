...
export default class App extends Component {
  ...
  _onRoomDidConnect = async () => {
    this.setState({status: 'connected'});
    //alert("Room Connected");
  }

   _onConnectButtonPress = async () => {
    await this.refs.twilioVideo.connect({ roomName: this.state.roomName, accessToken: this.state.token })
  ...
  _onParticipantAddedVideoTrack = ({participant, track}) => {
    ...
    for (const [key, value] of syncVideoTracks) {
      if (value.participantSid === participant.sid) {
        syncVideoTracks.delete(valueTrack.trackSid);
      }
    }
